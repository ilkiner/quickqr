...
import { getBlogPost, multilingualPosts } from "src/lib/blog";
...
function renderContent(raw: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("## ")) {
      elements.push(
        <h2 key={i} className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-10 mb-3">
          {line.slice(3)}
        </h2>
      );
      i++;
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h3 key={i} className="text-xl font-semibold text-gray-900 dark:text-gray-100 mt-8 mb-2">
          {line.slice(4)}
        </h3>
      );
      i++;
            ...
            <thead className="bg-gray-50 dark:bg-[#1a1a1a] font-semibold text-gray-900 dark:text-gray-100">
              <tr>
                {parseCells(header).map((c, ci) => (
                  <th key={ci} className="text-left px-4 py-3">{c}</th>
                ))}
              </tr>
            </thead>
              ...
              {body.map((row, ri) => (
                <tr key={ri}>
                  {parseCells(row).map((c, ci) => (
                    <td key={ci} className="px-4 py-3 text-gray-700 dark:text-gray-300">{c}</td>
                  ))}
                </tr>
              ))}
            </tbody>
    ...
    if (line.startsWith("- ")) {
      ...
      elements.push(
        <ul key={i} className="list-disc pl-6 space-y-2 my-4 text-gray-700 dark:text-gray-300">
          {items.map((item, ii) => (
    ...
    if (/^\d+\. /.test(line)) {
      ...
      elements.push(
        <ol key={i} className="list-decimal pl-6 space-y-2 my-4 text-gray-700 dark:text-gray-300">
          {items.map((item, ii) => (
            <li key={ii}>{inlineFormat(item)}</li>
          ))}
        </ol>
    ...
    // Paragraph
    elements.push(
      <p key={i} className="text-gray-700 dark:text-gray-300 leading-relaxed my-4">
        {inlineFormat(line)}
      </p>
    );
    i++;
...
export default function BlogPostContent({ slug }: { slug: string }) {
  ...
  const otherPosts = multilingualPosts
    .filter((p) => p.slug !== slug)
    .map((p) => ({ slug: p.slug, ...p[lang] }));
    ...
    <main className="bg-white dark:bg-[#0a0a0a] min-h-screen transition-colors duration-200">
      ...
      <article className="max-w-3xl mx-auto px-4 py-12 dark:text-gray-300">
        ...
        <div className="mt-14 bg-black dark:bg-[#141414] dark:border dark:border-white/10 rounded-3xl px-8 py-10 text-center text-white">
          <h2 className="text-2xl font-bold mb-3">{b.articleCtaTitle}</h2>
          <p className="text-gray-300 mb-6">{b.articleCtaDesc}</p>
          <Link
            href="/generate"
